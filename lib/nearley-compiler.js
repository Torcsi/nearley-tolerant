...
class NearleyCompiler{
    ...
    compileFile(sFileName,hOpts){
        //console.log("COMPILE FILE"+JSON.stringify(hOpts))
        ...
        try{
            ...
            //console.log("HTMLFILENAME:"+hOpts.rr);
            if( "rr" in hOpts){
                let sHTMLFileName
                if( hOpts.rr == "")
    ...
    createParser(hParserTable,oLexer,oInterpreter,hOptions)
    {
        ...
        for(let sStart in hStart){
            //console.log("SET START FOR "+sStart);
            if( sStart=="undefined"){
                if(aErrors1!=null){
                    aErrors1 = [];
                }
                aErrors1.push("nearley-compiler: severe format error, undefined start symbol found");
            }else{
