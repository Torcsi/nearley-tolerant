...
class NearleyCompiler{
        ...
        for(let i = 0;i<t.length;i++){
            ...
            if( typeof(vPostprocess) == "string"){
                if( !(vPostprocess in oInterpreter) ){
                    ...
                    r.postprocess = (data)=>{ console.log("### procstr:" +r.name+"->"+sFunctionName+" params "+data.length+this.dumpArray(data)); return fMethod.call(oInterpreter,data) };
                }
                ...
                if( vPostprocess.length == 0){
                    ...
                    if( !(sFunctionName in oInterpreter) ){
                        ...
                        //! parameter setup!                        
                        r.postprocess = (data)=>{ console.log("### procarr:" +r.name+"->"+sFunctionName+" params "+data.length+this.dumpArray(data)); return fMethod.call(oInterpreter,data) };
                    }
    ...
    linkGrammar(hTable,oLexer,oInterpreter){
        ...
        this.addInterpreter(hTable,oInterpreter,aErrors);
        return aErrors;
    }
}
