...
// TA 20180131 {+ added for JSON export

generate.json = function (parser, exportName) {
    var aOutput = [];
    aOutput.push("{");
    aOutput.push("    \"Lexer\":\""+ parser.config.lexer + "\",");
    aOutput.push("    \"Interpreter\":\""+ parser.config.interpreter + "\",");
    aOutput.push("    \"ParserRules\": ");
    aOutput.push("    "+serializeRules(parser.rules, generate.javascript.builtinPostprocessors)+",");
    aOutput.push("    \"ParserStart\": " + JSON.stringify(parser.start));
    aOutput.push("}")
    let sResult = aOutput.join("\n");
    try{
        let oGrammar = JSON.parse(sResult);
    }
    catch(eJSON){
      console.error((""+eJSON.stack).split("\n")[0]);
    }
    return sResult;
};
// tA 20180131 +}
