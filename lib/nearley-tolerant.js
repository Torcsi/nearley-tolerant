...
Parser.prototype.feed = function(chunk) {
    ...
    for (;;) {
        ...
        console.log("### token: "+(token==null?"FINISH":(token.type+":"+token.text)));
        if(!token)
        	break;
        ...
        for (var w = scannable.length; w--; ) {
            ...
            if (expect.test ? expect.test(value) :
                expect.type ? (expect.type === token.type || (lexer.isA && lexer.isA(expect.type,token.type,token.value)))
    ...
    if(token && token.type=="NONE" && this.options.tolerant){
    	this.results = this.finishTolerant();
    }else{
...
Parser.prototype.finishTolerant = function() {
    // Return the possible parsings
    var considerations = [];
    var start = this.grammar.start;
    var column = this.table[this.table.length - 1]
