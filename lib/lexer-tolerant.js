...
class Atom {
     ...
     */
    constructor(sName, vDefinition,bClone) {
        /** @private **/
        this._name = sName;
        ...
        if (vDefinition instanceof String || typeof(vDefinition)=="string") {
            ...
            if(bClone!==true)
                throw ("atom:"+sName+": no type defined"+(typeof (vDefinition)=="string")+":"+vDefinition);
        ...
        **/
        ...
        this._equivalenceDefs = {};
    }
    cloneObject()
    {
        let a = new Atom(this._name,null,true);
        for(let v in this){
    ...
    getValue(om) {
        // determine each part
        //console.log("### getValue for " + (om!=null?om[0]:null)+":"+this.name + ":" + this._sReParse);
        if (!this.isValueAtom)
            return "";
        /*
        for(var v in om){
        ...
        */
        ...
        //console.log(this.name+":"+"### result:" + JSON.stringify(m)+":"+this._sReParse);
        for(let v in m){
            if(!(v in om)){
                om[v] = m[v];
            }
            ...
            //console.log("### MATCH CHOICE?");
                for (let v in this._choice) {
                    ...
                    if (m[sChoiceName]) {
                        ...
                        m[0] = sOrig;
                        m[sChoiceName] = sAssembled
                        //console.log("### MATCH CHOICE:"+sChoiceName+":"+m[sChoiceName]+":"+m.weight);
                    }
                }
        ...
        //console.log("### substititions:"+this.name+":"+this._substitutes.length+":"+JSON.stringify(this._substitutes));
        for (let i = 0; i < this._substitutes.length; i++) {
            let h = this._substitutes[i];
            if( h._substituteId){
                //console.log("### substitition with same");
                ...
                //console.log("### substitition with object");
                if (sAssembled in h._substituteObject) {
                    sAssembled = h._substituteObject[sAssembled];
                }
            } else if (h._substituteFunction) {
                sAssembled = h._substituteFunction(m);
                //console.log("### substitition with function:result:"+sAssembled);
    ...
    checkSubstitution(aErrors) {
        ...
        if (this._composeString!==undefined) {
    ...
    */
    ...
    issue(sLevel,sMessage)
    {
        this.context(true,"",sLevel,sMessage);
        return this;
    }
    issueWhen(re,bMatch,sLevel,sMessage)
    {
        this.context(true,"",sLevel,sMessage,re,bMatch);
        return this;
    }
    /**
     * 
     * @param {boolean} bIn true if issue on given context, false if issue on not in given context
     * @param {string} vContextName name of context or array of names of context
     * @param {string} sLevel level: warning, error, hint, info
     * @param {string} sMessage the message itself
     * @param {*} re optional regexp for conditional
     */
    context(bIn,vContextName,sLevel,sMessage,re,bMatch)
    {
        if(this.context2issues===undefined)
            this.context2issue = {};
        let oContextWarning = { in: bIn, level:sLevel, message:sMessage};
        if(re!==undefined){
            oContextWarning.re = re;
            if(bMatch===undefined||bMatch===null)
                bMatch = true;
            oContextWarning.bReMatch = bMatch;
        }
        if(vContextName instanceof String){
            this.context2issue[vContextName] = oContextWarning;
        }else if(Array.isArray(vContextName)){
            for(let sContextName of vContextName){
                this.context2issue[sContextName] = oContextWarning;
            }
        }
        return this;
    }
    ...
    resolveReferences(hAtoms, aErrors) {
        ...
        if (this._overlaps) {
            this._overlapping = null;
            for (let v in this._overlaps ) {
                if (!(v in hAtoms)) {
     ...
     * @param {Array<string>} aErrors
     */
    checkReuse(aErrors,sOriginal,hUsed,bAdd) {
        if (sOriginal == null) {
            sOriginal = this.name;
            hUsed = {};
        }
        ...
        switch (this._type) {
            case AtomTypeSynonym:
                return this._synonym.checkReuse(aErrors, sOriginal, hUsed,false);
            case AtomTypeSequence:
                for (let i = 0; i < this._sequenceDef.length; i++) {
     ...
     * @param {Array<string>} aErrors
     */
    compileRegexp(aErrors,hConstants) {
        /**
        ...
        */
        ...
        switch (this._type) {
            case AtomTypeString:
                {            
                    let sRecognize = XRegExp.escape(this._string).replace(/\\ /g," ");
                    this._sReTest = sRecognize;
                    this._reTest = gbLONGESTMATCH ? XRegExp("(?:" + this._sReTest + ")", "gu") : XRegExp("^(?:" + this._sReTest + ")$", "u");
                    if (this.isValueAtom) {
            ...
            case AtomTypeRegexp:
                try {
                    ...
                    this._reTest = gbLONGESTMATCH ? XRegExp("(?:" + this._sReTest.replace(/^\^/,"").replace(/\$$/,"") + ")", "g"+(this._lexerRegexp.sOpt?this._lexerRegexp.sOpt.replace(/g/,''):"")) : XRegExp("^(?:" + this._sReTest.replace(/^\^/,"").replace(/\$$/,"") + ")$", this._lexerRegexp.sOpt);

                    if (this.isValueAtom) {
                        this._sReParse = "(?<" + this.name + ">" + this._sReTest + ")";
                    } else {
                        //console.log("OVERWRITE "+this.name)
            ...
            case AtomTypeSynonym:
                {
                    ...
                    oSynonym.compileRegexp(aErrors,hConstants);
            ...
            case AtomTypeChoice:
                {
                    ...
                    this._reTest = gbLONGESTMATCH ? XRegExp("(?:" + this._sReTest + ")", "gu") : XRegExp("^(?:" + this._sReTest + ")$", "u");
                    if(this.isValueAtom)
                        this._sReParse = "(?<"+this.name+">"+sReParse.substring(1) + ")";
                    else
                        this._sReParse = "("+sReParse.substring(1) + ")";
            ...
            case AtomTypeSequence:
                {
                    ...
                    //console.log("### SEQUENCE " + this.name+" re parse" +this._sReParse);
                    this._reParse = XRegExp("^" + this._sReParse + "$", "u");
    ...
    checkPartUnicity(aErrors, sOriginal, hReferred,bCheck) {
        if (hReferred == null)
            hReferred = {};
        if (sOriginal == null)
            sOriginal = this.name;
        ...
        switch (this._type) {
            ...
            case AtomTypeSynonym:
                {
                    let oSynonym = this._synonym;
                    oSynonym.checkPartUnicity(aErrors,sOriginal,hReferred,false);
                }
                break;
            case AtomTypeChoice:
                {
        ...
        for (let v in hAtoms) {
            ...
            if (oAtom._sReFind == this._sReFind) {
                if (!this.isOverlappingAtom(oAtom) )
                    aErrors.push("atom: " + this.name + " has the same recognition regexp, but not set to overlapping, with:" + oAtom.name +"("+this._sReFind+"~"+oAtom._sReFind+")");
            }
        }
    }
    get regexpSource() {
        return this._reTest.source;
    }
    /**
    test(s, at) {
        let atEnd = -1;
...
class LexicalUnits {
    ...
    /**
     * add all atoms of the "other" set of units; 
     * unlike "specialize" (that will override included atoms of the other set), local definitions are kept and inherited same definitions do not override
     * @param {LexicalUnits} lu 
     * @param {boolean} bTolerant false: report duplications, true: does not load duplications but does not report
     * @returns {[string]?}  null when OK, otherwise some errors of merging
     */
    addUnits(lu,bTolerant,sFile,aErrors)
    {
        if( lu == null)
            return null;
        for(let v in lu._constants){
            if( v in this._constants ){
                if( this._constants[v] != lu._constants[v])
                aErrors.push("lexerUnits:"+sFile+": duplicated constant:"+v);
            }
        }
        for(let v in lu._atoms){
            if( v in this._atoms ){
                if( !bTolerant ){
                    aErrors.push("lexerUnits:"+sFile+": duplicated atom:"+v);
                }
                continue;
            }
            this._atoms[v] = lu._atoms[v].cloneObject();
        }
        for(let v of lu._startDef )
            this._startDef.push(v);
        // merge common equivalents
        for(let otherEquivalents in lu._equivalenceDefs){
            if( !(otherEquivalents in this._equivalenceDefs))
                continue;
            // overlapping equivalenceDefs
            let hEquivalents = this._equivalenceDefs[otherEquivalents]
            for(let w in lu._equivalenceDefs[otherEquivalents]){
                hEquivalents[w] = true;
                if( !(w in this._equivalenceDefs )){
                    this._equivalenceDefs[w] = hEquivalents;
                }
            }
        }
        // add former equivalents
        for(let otherEquivalents in lu._equivalenceDefs){
            if( !(otherEquivalents in this._equivalenceDefs)){
                this._equivalenceDefs[otherEquivalents] = lu._equivalenceDefs[otherEquivalents];
            }
        }
        this._compiled = false;
    }
    ...
    addEquivalences(...p)
    {
        let h = {};
        for(let v of p){
            if( v in this._equivalenceDefs){
                for(let w in this._equivalenceDefs[v]){
                    h[w] = true;
                }
            }
            h[v] = true;
    ...
    setStart(sName) {
        //console.log("SET START "+sName)
        this._startDef.push(sName);
    }
    startAll(){
        this._startDef = [];
    ...
    _setAllEquivalences()
    {
        for(let v in this._equivalenceDefs){
            if( !(v in this._atoms))
                continue;
            let h = this._equivalenceDefs[v];
            for(let w in h){
                if( w in this._atoms )
                    this._atoms[w].overlaps(v);
            }
        }
    }
    compileStart(aErrors) {
        //console.log("COMPILESTART:");
        if (this._startDef.length == 0) {
        ...
        this._start = [];
        let hAdded = {};
        for (let i = 0; i < this._startDef.length; i++) {
            let sName = this._startDef[i];
            if( sName in hAdded )
                continue;
            hAdded[sName] = true;
            //console.log("### Start atom:"+sName)
            let oAtom = this._atoms[sName];
            this._start[i] = oAtom;
    ...
    compileAny(aErrors) {
        ...
        try{
            ...
            aErrors.push("compileAny: invalid regexp");
        }
    }
    ...
    compileRegexp(aErrors) {
        let hParts = {};
        for(let v in this._constants){
            hParts[v] = new XRegExp(this._constants[v]);
        }

        for (let sAtom in this._atoms) {
            let oAtom = this._atoms[sAtom];
            oAtom.compileRegexp(aErrors,hParts);
        }
        return aErrors.length == 0;
    }
    ...
    findFirst(s, at, match)
    {
        ...
        while ((m = XRegExp.exec(s, this._reStart, at)) != null) {
            ...
            if( gbLONGESTMATCH ){
                /** longest match case **/
                let longestMatchLength = m[0].length-1;
                //console.log("###  TESTME "+foundAt+" longestmatch "+longestMatchLength)
                for (let i = 0; i < this._start.length; i++) {
                    
                    //console.log(m + ":" + i + ":" + this._start[i]._reTest.test(s,-1));
                    let m1;
                    
                    //console.log("###  FINDFIRSTWHICH ["+m[0]+"] TEST for " + this._start[i]._reTest.source)
                    if ((m1=XRegExp.exec(s, this._start[i]._reTest, foundAt))!=null) {
                        let innerFoundAt = this._start[i]._reTest.lastIndex - m1[0].length;
                        //console.log("###  FOUNDFIRST "+this._start[i].name+":"+m1[0].length + " comparable to "+longestMatchLength+":" +foundAt+"=>"+this._start[i]._reTest.lastIndex+":"+m1[0].length);
                        if (innerFoundAt == foundAt && m1[0].length>longestMatchLength) {
                            //console.log(" ### Lexer.FOUNDWHICH " + this._start[i].name);
                            longestMatchLength = m1[0].length;
                            match.at = foundAt;
                            match.length = m1[0].length;
                            match.end = match.at + longestMatchLength;
                            match.text = m1[0];
                            match.atom = this._start[i];
                            match.type = match.atom.name;
                            /*
                            let sValue = match.atom.getValue(m1);
                            console.log("### GETVALUE for "+match.atom.name+":"+m1[0]+":"+this._start[i].isValueAtom);
                            */
                            if( this._start[i].isValueAtom)
                                match.getValue = function () { /*console.log("GETTING KNOWN VALUE:"+sValue); */return match.atom.getValue(m1); }
                            else
                                match.getValue = null;
                ...
                //console.log("### AFTER FINDFIRST:"+match.at)
                //process.exit();
                return match;
            }else{
                /** Exact match case **/
    ...
    findNext(s, at, match) {
        ...
        //console.log("### Lexer.FINDNEXT >>>" + s.substring(at,at+10) + "<<<...:" + at + " "+this._reAny.source);
        while ((m = XRegExp.exec(s, this._reAny, at)) != null) {
            ...
            if( gbLONGESTMATCH){
                let longestMatchLength = m[0].length-1;
                //console.log("###  TESTME "+foundAt)
                for (let i = 0; i < this._aAny.length; i++) {
                    let m1;
                    
                    //console.log(m + ":" + i + ":" + this._start[i]._reTest.test(s,-1));
                    //console.log(m[0] + ":" + i + ":" + this._aReWord[i].test(m[0],-1));
                    //console.log(" ###  Lexer.FINDNEXTWHICH ["+m[0]+"] TEST for " + this._aAny[i]._reTest.source)
                    if ((m1=XRegExp.exec(s, this._aAny[i]._reTest, foundAt))!=null) {
                        let innerFoundAt = this._aAny[i]._reTest.lastIndex - m1[0].length;
                        
                        //console.log("###  FOUNDNEXT "+this._aAny[i].name+":"+m1[0].length + " comparable to "+longestMatchLength+":"+foundAt+"=> at:"+this._aAny[i]._reTest.lastIndex+", len:"+m1[0].length+", hit["+m1[0]+"]")
                        if (innerFoundAt == foundAt && m1[0].length>longestMatchLength) {
                            //console.log(" ### Lexer.FOUNDWHICHNEXT " + this._aAny[i].name+":"+m1[0]);
                            longestMatchLength = m1[0].length;
                            match.at = foundAt;
                            match.length = m1[0].length;
                            match.end = match.at + longestMatchLength;
                            ...
                            /*
                            let sValue = match.atom.getValue(m1);
                            console.log("### GETVALUE for "+match.atom.name+":"+m1[0]+":"+this._aAny[i].isValueAtom+"=>"+sValue);
                            */
                            if( match.atom.isValueAtom)
                                match.getValue = function () { /*console.log("GETTING KNOWN VALUE:"+sValue+(m1==null?"NULL!!":"NOTNULL"));*/ return match.atom.getValue(m1); }
                            else
                                match.getValue = null;
                    ...
                    if ((m1 = XRegExp.exec(m[0], this._aAny[i]._reTest, -1)) != null) {
                        if (m[0].length == m1[0].length) {
                            ...
                            if( this._aAny[i].isValueAtom)
                                match.getValue = function () { return match.atom.getValue(m1); }
                            else
                                match.getValue = null;
                            return match;
                        }
    ...
    isA(t1,t2,sValue,oToken)
    {
        ...
        if(!v)
            return null;
        //console.log("### isA:"+t1+":"+t2+":"+(t2 in v)+":ON:"+sValue);
        if(! (t2 in v ) )
            return null;
        // test matching
        /**
         * @type {Atom}
         */
        let expectedAtom = this._atoms[t1];
        
        let m = XRegExp.exec(sValue, expectedAtom._reTest, 0)
        if(!m) // XRegExp.test(sValue,expectedAtom._reTest))
            return false;
        let newToken = Object.assign({},oToken);
        if( expectedAtom.isValueAtom)
            newToken.getValue = function () { return expectedAtom.getValue(m); };
        else
    ...
    */
    empty()
    {
        return Object.keys(this._atoms).length==0;
    }
}
...
class LexicalParser {
    ...
    logText()
    {
        ...
        if(this._log.length==0)
            return "";
        for(let oToken of this._log){
            aRes.push("{TOKEN:"+oToken.type+",expected:"+oToken.expected+",atom:"+(oToken.atom==null?"null":oToken.atom.name)+",text:"+oToken.text);
        }
        return aRes.join("\n")
    }
    _logPush(oToken)
    {
        this._log.push(oToken)
    }
    setLexer(oUnits){
        this.units = oUnits;
    }
    ...
    reset(sString,i)
	{
		if( i!=null){
            ...
            this.at = i.at;
            //console.log("### reset "+this.at);

            this.findFirst(this.at);
		}else{
			this.sString = sString==null?this.sString:sString;
            this.at = 0;
            this.end = 0;
            this.aBlocked = new Int8Array(this.sString.length);
            this.l = this.sString.length;
            this.match=null;
		}
    }
    save(){
    ...
    findFirst(at) {
        ...
        while (this.units.findFirst(this.sString, at, match)) {
            //console.log("### FINDFIRST: "+match.type+":"+match.text);
            let nextFree = this.getLastBlocked(match.at, match.end);
            if (nextFree == match.end) {
                // not a blocked portion
                return match.at;
            }
            //console.log("### NEXTFREE is:"+nextFree+" locked "+match.at+"-"+match.end+":"+JSON.stringify(this.aBlocked));
            at = nextFree+1;
        }
        this.match = null;
        return this.l;
    ...
    isA(t1,t2,sValue,oToken){
        return this.units.isA(t1,t2,sValue,oToken);
    }
    next(hExpected)
    {
        this._unparsed = null;
        let r = this._next(hExpected);
        if( this.logOn && r!=null ){
            r.expected = JSON.stringify(hExpected);
            this._logPush(r);
        }
        //console.log("### NEXT finished "+(r!=null?r.type:"")+(hExpected!='undefined'?JSON.stringify(hExpected):""));
        if(r){
            //console.log("### "+r.type+":"+r.text+"~"+this.at+"<?"+this.l)
            if(r.type=="NONE" && this.at>=this.l){
                this._unparsed = r;
                return null;
            }
        }
    ...
    _next(hExpected) {
        //console.log("### NEXT "+this.at+":"+this.sString.substr(this.at,10));
        if (this.match != null) {
            // prepread

            //console.log("### PREREAD "+this.match.type);
            /*
        ...
        */
        ...
        for(;;){
          ...
          //console.log("### AFTERNEXTREAD "+this.end+"/"+this.l+":"+match.type);
          /*
          for (var v in match) {
        ...
        //let bNonMatch = false
        if (match.at == this.at) {
            //console.log("### FOUND AT "+this.at+" until "+match.end+" "+match.type);
            ...
            // check, if by chance, the arrived one is any like isA
            for(let sExpected in hExpected){
                //console.log(sExpected+":"+match.type+":["+match.text+"]");
                let oNewToken = this.isA(sExpected,match.type,match.text,match);
                if( oNewToken ){
                    this.at = match.end;
                    //console.log("### like received "+this.at);
...
function atom(...params){
    if( oLexer != null){
        if( params[0] in oLexer._atoms ){
            console.error("lexer: redefinition of "+params[0]);
            return oLexer._atoms[params[0]];
        }
    }
...
function constant(k,v)
{
    oLexer.addConstant(k,v);
}

function equivalences(...params)
{
    if( oLexer != null){
        oLexer.addEquivalences(...params);
    }
}
...
module.exports = { Lexer:LexicalUnits, Atom, LexicalParser, atom, constant,use, regexp, equivalences }