  ...
  function(){
    ...
    describe("Substitute",
      function(){
        ...
        **/
        it.skip("Empty array substitute (cannot be reported)",
          function(){
            let myLexer= new Lexer();
            use(myLexer);
            atom("my","my");
            ...
            myLexer.setStart("var");
            let aErrors = myLexer.compile();
            assert.ok(aErrors && aErrors.length > 0 && aErrors[0].indexOf("empty array substitut")>=0,"empty  array substitution is not reported "+JSON.stringify(aErrors));
          }
        ),
        it("Numeric substitute",
          function(){
            let myLexer= new Lexer();
            use(myLexer);
            atom("my","my").substitute("$1");
            myLexer.setStart("my");
            let aErrors = myLexer.compile();
            assert.ok(aErrors && aErrors.length > 0 && aErrors[0].indexOf("numeric group")>=0,"invalid numeric group substitution is not reported "+JSON.stringify(aErrors));
          }
        )
      }
    );
